...
def pay(request, id):
    ...
    if request.method == 'POST':
        ...
        amount = int(amount)
        phone_number = request.POST.get('customer-number')
        account_ref = "PAYMENT_1"
        transaction_description = "Paying for a product"
        transaction = mpesa_client.stk_push(phone_number, amount, account_ref, transaction_description, stk_push_callback_url)
